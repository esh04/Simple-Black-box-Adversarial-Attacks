...
def SIMBA(true_preds, images, true, model, eps=0.2):
    ...
    for i in tqdm.tqdm(range(len(true_preds))):
        ...
        input_img = images[true_preds[i]].unsqueeze(0).to(device)
        label = true[true_preds[i]].unsqueeze(0).to(device)
        model = model.to(device)
        out = model(input_img)
        out = F.softmax(out,dim = 1)

        target = (out.topk(10,dim=1)[1].squeeze().tolist()[-1])

        # print device 
        # print(target.device.type)
        # print(model.device.type)
        # print(images[true_preds[i]].device.type)
        # print(true[true_preds[i]].device.type)

        # model to cpu
        model = model.to(torch.device("cpu"))


        adv_img_,iter_,init_prob_act_,fin_prob_act_,init_prob_adv_,fin_prob_adv_ = SIMBA_single_tar(images[true_preds[i]],true[true_preds[i]], target , model, eps)
    ...
    return adv_img, iter, avg_iter, avg_diff_init_pred, avg_diff_final_pred
...
adv_img2, iter2, avg_iter2, avg_diff_init_pred2, avg_diff_final_pred2 = SIMBA(true_preds[:100], images, true, model, eps=0.2)

# save as pickle file
with open('./adv_img_tar2.pkl', 'wb') as f:
    pickle.dump(adv_img2, f)

with open('./iter_tar2.pkl', 'wb') as f:
    pickle.dump(iter2, f)

with open('./avg_iter_tar2.pkl', 'wb') as f:
    pickle.dump(avg_iter2, f)

with open('./avg_diff_init_pred_tar2.pkl', 'wb') as f:
    pickle.dump(avg_diff_init_pred2, f)

with open('./avg_diff_final_pred_tar2.pkl', 'wb') as f:
