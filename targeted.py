...
def SIMBA_single(x, target, model, epsilon=0.2, num_iters=10000):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    for i in range(num_iters):
        ...
        loss = F.nll_loss(torch.log(probs), target.unsqueeze(0))
        loss.backward()

        grad = delta.grad.detach().clone().view(x.size())
        delta.grad.zero_()

        if grad.norm(p=2) > 0:
            delta_norm = grad.norm(p=2)
            delta /= delta_norm / epsilon
            add_vec = x + delta.view(x.size())
            probs = F.softmax(model(add_vec.to(device)))
            new_prob = probs[0][target]
    ...
    return x.squeeze(), i+1, pred_label, def_prob


def SIMBA(target, true_preds, images, true, model, eps=0.2):
    adv_img = []
    iter = []
    diff_init_pred = []
    diff_final_pred = []
    ...
    for i in tqdm.tqdm(range(len(true_preds))):
        adv_img_,iter_,new_class_,init_prob_act_,fin_prob_act_,init_prob_adv_,fin_prob_adv_ = SIMBA_single(images[true_preds[i]], true[true_preds[i]], model, target, eps)
        adv_img.append(adv_img_)
        iter.append(iter_)
        new_class.append(target)
        ...
        diff_init_pred.append(init_prob_act_[target_idx]-init_prob_adv_[target_idx])
        diff_final_pred.append(fin_prob_act_[target_idx]-fin_prob_adv_[target_idx])
